...
runslow = TRUE
resfn = "Pleuro_DIVALIKExj_M2.Rdata"
if (runslow)
{
...
###########################################
model_name = "DIVA+x+j_M2_strat"
# res = resDEC

#######################################################
# Plot ancestral states - DIVA
#######################################################
pdffn = paste0("DIVAxj", model_name, "_v1.pdf")
...
# Pie chart
plot_BioGeoBEARS_results(results_object, analysis_titletxt, addl_params=list("j"), plotwhat="pie", label.offset=0.45, tipcex=0, statecex=0.45, splitcex=0.45, titlecex=0.8, plotsplits=TRUE, cornercoords_loc=scriptdir, include_null_range=TRUE, tr=tr, tipranges=tipranges)
...
#######################################################
# Stochastic mapping on DIVA M2 stratified 
#######################################################
...
pdf(file=pdffn, width=12, height=24)
...
plot_BioGeoBEARS_results(results_object=resmod, analysis_titletxt="Stochastic map", addl_params=list("j"), label.offset=0.45, plotwhat="text", cornercoords_loc=scriptdir, root.edge=TRUE, colors_list_for_states=colors_list_for_states, skiptree=FALSE, show.tip.label=TRUE)
